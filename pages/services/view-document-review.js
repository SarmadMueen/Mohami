...
const ViewDocumentReviewPage = () => {
    ...
    useEffect(() => {
        if (review) {
            if (review.key_findings) {
                setActiveTab('notes');
            } else if (review.next_action_required) {
                setActiveTab('review-status');
            } else {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            router.push('/services/add-document-review');
        } catch (error) {
    ...
    const handlePrint = () => {
        // Create a print window with only the review content
        ...
        const printContent = `
            ...
            <body>
                ...
                <div class="print-section">
                    <h2 class="print-section-title">إعدادات المراجعة</h2>
                    <div class="print-grid">
                        <div class="print-item">
                            <div class="print-label">عنوان المستند</div>
                            <div class="print-value">${review.document_title || '-'}</div>
                        </div>
                        <div class="print-item">
                        ...
                        ${review.document_fees && parseFloat(review.document_fees) > 0 ? `
                        <div class="print-item">
                            <div class="print-label">رسوم المراجعة</div>
                            <div class="print-value">${parseFloat(review.document_fees).toFixed(2)} دينار</div>
                        </div>
                        ` : ''}
                        ${review.next_action_required ? `
                        <div class="print-item">
                            <div class="print-label">الإجراء المطلوب</div>
                            <div class="print-value">${review.next_action_required}</div>
                        </div>
                        ` : ''}
                        ${review.action_deadline ? `
                        <div class="print-item">
                            <div class="print-label">تاريخ انتهاء الإجراء</div>
                            <div class="print-value">${new Date(review.action_deadline).toLocaleDateString('en-US')}</div>
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                ${review.key_findings ? `
                <div class="print-section">
                    <h2 class="print-section-title">الملاحظات</h2>
                    <div class="print-text">${review.key_findings}</div>
                </div>
                ` : ''}
            </body>
            </html>
    ...
    return (
        <Layout>
            <Head>
                <title>عرض مراجعة أوراق قانونية - مكتب المحامي</title>
            </Head>
            <div className="view-review-page">
                <div className="breadcrumbs">
                    <Link href="/services/legal-services">الخدمات القانونية</Link> / 
                    <Link href="/services/add-document-review">مراجعة أوراق قانونية</Link> / 
                    عرض المراجعة
                </div>
                <style jsx>{`
                    .view-review-page {
                    ...
                    /* Override any global CSS that might affect sidebar */
                    .view-review-page .sidebar * {
                        color: inherit;
                    }
                ...
                <div className="page-container">
                    <div className="main-content">
                        ...
                        {/* إعدادات المراجعة */}
                        <div className="info-section" id="review-settings">
                            <div className="info-grid">
                                <div className="info-card">
                                    <div className="detail-row">
                                        <span className="detail-label">عنوان المستند:</span>
                                        <span className="detail-value">{review.document_title || '-'}</span>
                                    </div>
                                </div>
                                <div className="info-card">
                                    <div className="detail-row">
                                    ...
                                    <div className="info-card" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', border: '2px solid #3b82f6' }}>
                                        <div className="detail-row">
                                            ...
                                            <span className="detail-value" style={{ color: '#1e40af', fontWeight: '700', fontSize: '1.125rem' }}>
                                                {parseFloat(review.document_fees).toFixed(2)} دينار
                                            </span>
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="tabs-container">
                            <div className="tabs-nav">
                                {(review.next_action_required || review.action_deadline) && (
                                    <button
                                        className={`tab-button ${activeTab === 'review-status' ? 'active' : ''}`}
                                        onClick={() => setActiveTab('review-status')}
                                    >
                                        حالة المراجعة
                                    </button>
                                )}
                            ...
                            {activeTab && (
                                <div className="tab-content">
                                    {activeTab === 'review-status' && (review.next_action_required || review.action_deadline) && (
                                        <div className="tab-panel">
                                            <div className="info-grid">
                                                <div className="info-card">
                                                    <div className="detail-row">
                                                        <span className="detail-label">الإجراء المطلوب:</span>
                                                        <span className="detail-value">{review.next_action_required || '-'}</span>
                                                    </div>
                                                </div>
                                                <div className="info-card">
                                                    <div className="detail-row">
                                                        <span className="detail-label">تاريخ انتهاء الإجراء:</span>
                                                        <span className="detail-value">
                                                            {review.action_deadline
                                                                ? new Date(review.action_deadline).toLocaleDateString('en-US')
                                                                : '-'}
                                                        </span>
                                                    </div>
                                                </div>
                                    ...
                                    {activeTab === 'notes' && review.key_findings && (
                                        <div className="tab-panel">
                                            <p style={{ 
                                                color: '#1e293b', 
                                                lineHeight: '1.8',
                    ...
                    <div className="sidebar">
                        ...
                        <button 
                            ...
                            onClick={() => router.push(`/services/add-document-review?edit=${id}`)}
                        >
                            <FiEdit />
                            تعديل المراجعة
                        </button>
...
export default withAuth(ViewDocumentReviewPage);
