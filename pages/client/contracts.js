...
const ContractsPage = () => {
  ...
  useEffect(() => {
    const fetchContracts = async () => {
      try {
        ...
        // Fetch contracts for this admin (created by admin or lawyers under same admin)
  ...
  useEffect(() => {
    ...
    if (selectedContractType) {
      filtered = filtered.filter(contract => contract.contract_type === selectedContractType);
    }
    ...
    setCurrentPage(1);
  }, [searchQuery, selectedContractType, sortOrder, contracts]);
  ...
  const handleFileInputChange = (e, attachmentNumber) => {
    const file = e.target.files[0];
    if (attachmentNumber === 1) setSelectedImage1(file);
    if (attachmentNumber === 2) setSelectedImage2(file);
    if (attachmentNumber === 3) setSelectedImage3(file);
    if (attachmentNumber === 4) setSelectedImage4(file);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedImage1) {
        const fileName1 = `${Date.now()}_1_${selectedImage1.name}`;
        attach1URL = await uploadFile(selectedImage1, `contracts/${fileName1}`);
      }

      if (selectedImage2) {
        const fileName2 = `${Date.now()}_2_${selectedImage2.name}`;
        attach2URL = await uploadFile(selectedImage2, `contracts/${fileName2}`);
      }

      if (selectedImage3) {
        const fileName3 = `${Date.now()}_3_${selectedImage3.name}`;
        attach3URL = await uploadFile(selectedImage3, `contracts/${fileName3}`);
      }

      if (selectedImage4) {
        const fileName4 = `${Date.now()}_4_${selectedImage4.name}`;
        attach4URL = await uploadFile(selectedImage4, `contracts/${fileName4}`);
      }
      ...
      const { data: insertedData, error } = await supabase.from('contract').insert([
        {
          ...
          contract_amount: formData.contractAmount.trim(),
          notes: formData.notes.trim(),
          attach_1: attach1URL || null,
          attach_2: attach2URL || null,
          attach_3: attach3URL || null,
          attach_4: attach4URL || null,
          admin_id: adminId,
          client_id: userId,
        },
      ]).select();
      ...
      // Refresh the list
      const { data: contractDataNew, error: contractErrorNew } = await supabase
        .from('contract')
        .select('*')
        .eq('admin_id', adminId)
          ...
          <div className="header-section">
            <div className="search-container">
              <form onSubmit={handleSearch} className="search-form">
                ...
                <input
                  type="text"
                  placeholder="البحث باسم الموكل, رقم العقد"
                  value={searchQuery}
                ...
                <table className="contracts-table">
                  <thead>
                    <tr>
                      ...
                      <th className="table-header">نوع العقد</th>
                      <th className="table-header">رقم العقد</th>
                      <th className="table-header">مبلغ العقد</th>
                    ...
                    {currentContracts.length > 0 ? (
                      currentContracts.map((contract, index) => (
                        <tr key={index} className="table-row">
                          <td className="table-cell">{contract.client_name || '-'}</td>
                          <td className="table-cell">{contract.contract_type || '-'}</td>
                          <td className="table-cell">{contract.contract_number || '-'}</td>
                          <td className="table-cell">{contract.contract_amount || '-'}</td>
                          <td className="table-cell">{formatDate(contract.start_date)}</td>
                          <td className="table-cell">{formatDate(contract.end_date)}</td>
                          <td className="table-cell">{contract.notes || '-'}</td>
                ...
                {totalPages > 1 && (
                  <div className="pagination-container">
                    <div className="pagination-info">
                      عرض {indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredContracts.length)} من {filteredContracts.length} عقد
                    </div>
            ...
            <div className="popup-overlay" onClick={() => setShowPopup(false)}>
              <div className="popup-container-wide" onClick={(e) => e.stopPropagation()}>
                <div className="popup-header">
                  <h3 className="popup-title">إضافة عقد</h3>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="popup-content">
                    ...
                    <div className="form-right">
                      <div className="input-wrapper">
                        <label htmlFor="attachment1">المرفق 1</label>
                        <input
                          type="file"
                          id="attachment1"
                          name="attachment1"
                      ...
                      <div className="input-wrapper">
                        <label htmlFor="attachment2">المرفق 2</label>
                        <input
                          type="file"
                          id="attachment2"
                          name="attachment2"
                      ...
                      <div className="input-wrapper">
                        <label htmlFor="attachment3">المرفق 3</label>
                        <input
                          type="file"
                          id="attachment3"
                          name="attachment3"
                      ...
                      <div className="input-wrapper">
                        <label htmlFor="attachment4">المرفق 4</label>
                        <input
                          type="file"
                          id="attachment4"
                          name="attachment4"
...
export default withAuth(ContractsPage, ['can_view_clients']);
